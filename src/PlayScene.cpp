...
void PlayScene::GUI_Function()
{
	...
	static int targetPositon[] = { m_pTarget->getGridPosition().x, m_pTarget->getGridPosition().y };
	if (ImGui::SliderInt2("Target Position", targetPositon, 0, Config::COL_NUM - 1))
	{
		//Row ajustment
		if (targetPositon[1] > Config::ROW_NUM - 1)
		{
			targetPositon[1] = Config::ROW_NUM - 1;
		}
		SDL_RenderClear(Renderer::Instance()->getRenderer());
		m_pTarget->getTransform()->position = m_getTile(targetPositon[0], targetPositon[1])->getTransform()->position + offset;
		m_pTarget->setGridPosition(targetPositon[0], targetPositon[1]);
		m_computeTileCosts();
		SDL_SetRenderDrawColor(Renderer::Instance()->getRenderer(), 255, 255, 255, 255);
...
void PlayScene::m_buildGrid()
{
	...
	//create a reference for each tile to its neighbours
	for (int row = 0; row < Config::ROW_NUM; ++row)
	{
		for (int col = 0; col < Config::COL_NUM; ++col)
		{
